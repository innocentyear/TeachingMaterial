...
namespace TeachingMaterial.Controllers
{
    ...
    public class BookSubscriptionController : Controller
    {
        ...
        public ActionResult Index(string sortOrder, int? semesterID, int? gradeID, int? departmentID, int? majorID, CourseType? courseType, string courseName,bool? isSubmmit, int? pageSize, int page = 1)
        {
            ...
            var _semesterCourse = db.SemesterCourses.Include(x => x.GradeMajor).Include(x => x.BookSubscriptions);
            //过滤操作
            if (semesterID != null)
            {
      ...
      */
       ...
       */
        ...
        //批量设置提交或取消提交教材征订信息。适用于批量设置 学期课程提交 和未提交两种情况
        public int MassSetSubmmit(bool expected = true, params int[] selectedIds) //必须为int型，负责find()会出现主键的类型不匹配
        {
            int number = 0;
            if (selectedIds != null)
            {
                foreach (var semesterCourseId in selectedIds)
                {
                    ...
                    if (semesterCourse != null)
                    {
                        if (expected)
                            ...
                            semesterCourse.SubmmitState = false;
